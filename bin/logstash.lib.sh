...
install_deps() {
  ...
  elif [ -f "$basedir/logstash-contrib.gemspec" ] ; then
    program="$basedir/gembag.rb"
    set -- "$basedir/logstash-contrib.gemspec"
    exec $RUBYCMD "$basedir/gembag.rb" "$@"
  else
    echo "Cannot install dependencies; missing logstash.gemspec (or logstash-contrib.gemspec). This 'deps' command only works from a logstash git clone."
  fi
}
